...
:::message
CUDA Driverを更新した際には、再生成する必要がある旨、記載あり。後の項で、CDIの中身を掲載しているが、個々のライブラリ等のパスを指定してコンテナ側に共有しているので、ホスト側のライブラリ等バージョンが変わってファイルが存在しなくなると、当然コンテナ側からもアクセス出来なくなる。
...
:::details CDI nvidia.yaml

```yaml:/etc/cdi/nvidia.yaml
---
cdiVersion: 0.5.0
containerEdits:
  deviceNodes:
